...
function googleAJAX() {
    ...
    setTimeout(function() {
    //run code here
    let name = $('p').text(data[0].placement_type_name);
    let adType = $('p').text(data[0].placement_description);
    let ctr = $('p').text(data[0].click_through_rate_as_percentage);
    $('#google').html("name: "+ name +"<br>AD Type: "+adType+ "<br>Click Through Rate: " + ctr);
                }, 100);
    },
    error:function(error){
    },
...
function businessInfoAJAX() {
    ...
    setTimeout(function() {
    //run code here
    let name = data.business_name;
    let image = "<img src=" + data.business_image_src + "/>";
    $('#biz').html(image).append(name);
                }, 200);        
    },
    error:function(error){
    },
